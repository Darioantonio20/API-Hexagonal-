...
export class MongoStoreRepository implements StoreRepository {
	...
	async findAll(): Promise<Store[]> {
		...
		return stores.map(store => new Store(store.id, store.name, store.address));
	}

	async create(store: Store): Promise<Store> {
		const newStore = new StoreModel(store);
		await newStore.save();
		return new Store(newStore.id, newStore.name, newStore.address);
	}
	...
	async update(id: string, store: Partial<Store>): Promise<Store | null> {
		...
		return updatedStore ? new Store(updatedStore.id, updatedStore.name, updatedStore.address) : null;
	}
